...
export class MtaBuildCommand {
    ...
    public async mtaBuildCommand(selected: any): Promise<void> {
        let response = await Utils.execCommand(MBT_COMMAND, ["-v"], {cwd: homeDir});
        ...
        if(selected){
            ...
            const mtaYamlFilesPaths = await vscode.workspace.findFiles('**/mta.yaml', '**/node_modules/**');
            ...
            if(len === 0){
                ...
                const selectionItems: SelectionItem[] = await SelectionItem.getSelectionItems(mtaYamlFilesPaths);
