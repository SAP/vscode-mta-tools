...
export class MtarDeployCommand {
    ...
    public async mtarDeployCommand(selected: any): Promise<void> {
        ...
        if(selected){
            ...
            const mtarFilesPaths = await vscode.workspace.findFiles('**/*.mtar', '**/node_modules/**');
            ...
            if(len === 0){
                ...
                const selectionItems: SelectionItem[] = await SelectionItem.getSelectionItems(mtarFilesPaths);
                const userSelection: vscode.QuickPickItem = await Utils.displayOptions(inputRequest, selectionItems);
    ...
    private async isLoggedInToCF(): Promise<boolean>{
        const results = await Promise.all([Utils.getConfigFileField("OrganizationFields"), Utils.getConfigFileField("SpaceFields")]);
